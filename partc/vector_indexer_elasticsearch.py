...
try:
    ...
    print("Milvus not installed. Installing...")
...
class MilvusVectorIndexer:
    ...
    def __init__(self, embeddings_dir: str, db_path: str = None):
        ...
        # Connect to Milvus Lite (local file-based, no server needed)
        try:
            connections.connect(
                alias="default",
                uri=db_path  # Local file path for Milvus Lite
            )
    ...
    def create_collections(self, embedding_dim: int, available_models: List[str]):
        ...
        try:
            # Create parent collection schema - ADD DUMMY VECTOR FIELD (Milvus 2.6+ requirement)
            parent_fields = [
        ...
        try:
            ...
            try:
                parent_data = [
                    ...
                    [[0.0]],  # dummy_vector - required for Milvus 2.6+
                ]
